...
export class CountriesDatamapComponent implements OnInit {
  ...
  generateMap() {
    ...
    if (this.parentWidth) {
      if (this.mapContainer.nativeElement.parentElement) {
        // parentElement
        this.availableWidth = this.mapContainer.nativeElement.parentElement.offsetWidth;
      } else {
        this.availableWidth =
    ...
    const height = this.availableWidth * SIZE_FACTOR;
    ...
    this.renderedMap = new DataMap({
      ...
      geographyConfig: {
        ...
        popupTemplate: (geography: DataMapGeographyData, data) => {
